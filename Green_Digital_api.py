import re
"""
rio_erp_api.py — GREEN DIGITALS ERP v2.0
...
if not _LOG_FILE:
    if _platform.system() == "Windows":
        ...
        logger.info("=== GREEN DIGITALS ERP v2.0 — Render — check Logs tab ===")
...
HTML_FILE  = os.environ.get("HTML_FILE", "Green_Digital_ERP.html")

# ── Startup diagnostics ──
logger.info("=" * 60)
logger.info("GREEN DIGITALS ERP v2.0 — STARTING UP")
...
def ensure_default_users():
    ...
    try:
        existing = col("green_digital_users").find_one({"username": "admin"})
        if not existing:
            col("green_digital_users").insert_one({
                "username": "admin",
            ...
            if update_fields:
                col("green_digital_users").update_one({"username": "admin"}, {"$set": update_fields})
    except Exception as e:
...
def next_invoice_no(inv_type: str, fy: str) -> str:
    ...
    if inv_type == "GST":
        pfx = "RU" if is_rb else "R"
        rgx = r"^RU\d" if is_rb else r"^R\d"
        skip = 2 if is_rb else 1
        pipeline = [
            {"$match": {
                "InvoiceNo": {"$regex": rgx},
        ...
        pfx2 = "RUN" if is_rb else "RN"
        rgx2 = r"^RUN" if is_rb else r"^RN"
        skip2 = 3 if is_rb else 2
        pipeline = [
            {"$match": {
...
def init_counters():
    ...
    collections = [
        ...
        ("notes", "Id"), ("followups", "Id"), ("green_digital_clients", "Id"),
...
app = FastAPI(title="GREEN DIGITALS ERP v3.0", lifespan=lifespan)
...
async def serve_ledger(request: Request):
    ...
    LEDGER_FILE = "Green_Digital_ERP_ledger.html"
...
def list_companies():
    return JSONResponse({"companies":[{"id":"rio","name":"GREEN DIGITALS"},{"id":"rainbow","name":"RAINBOW UMBRELLAS"}]})
...
def sync_rainbow():
    ...
    results={}
    for c in ["clients","green_digital_clients","products","categories","sizes"]:
        try:
...
async def post_sales(request: Request):
    ...
    # Auto-add client
    if doc["Customer"]:
        shared_col("green_digital_clients").update_one(
            {"ClientName": doc["Customer"]},
            {"$setOnInsert": {"Id": next_id("green_digital_clients"), "ClientName": doc["Customer"], "createdBy": doc.get("createdBy", "")}},
            upsert=True
        )
    # Ledger credits
    payments = [
...
async def get_clients():
    ...
    if not await ensure_db(): return JSONResponse(content=[], status_code=503)
    rows = list(shared_col("green_digital_clients").find({}, {"_id": 0, "ClientName": 1}).sort("ClientName", ASCENDING))
...
async def get_rio_clients(q: Optional[str] = Query(None)):
    ...
    rows = list(shared_col("green_digital_clients").find(query, {"_id": 0}).sort("ClientName", ASCENDING))
...
async def post_clients(request: Request):
    ...
    if name:
        shared_col("green_digital_clients").update_one(
            {"ClientName": name},
            {"$setOnInsert": {"Id": next_id("green_digital_clients"), "ClientName": name, "createdBy": created_by}},
            upsert=True
        )
    return ok()
...
async def delete_client(client_name: str):
    ...
    shared_col("green_digital_clients").delete_one({"ClientName": {"$regex": f"^{re.escape(name)}$", "$options": "i"}})
...
async def billing_status():
    cc = shared_col("green_digital_clients").count_documents({})
...
async def billing_post_customer(request: Request):
    ...
    existing = shared_col("green_digital_clients").find_one({"ClientName": name})
    update_doc = {
    ...
    if existing:
        shared_col("green_digital_clients").update_one({"ClientName": name}, {"$set": update_doc})
        return ok({"success": True, "id": existing["Id"]})
    else:
        new_id = next_id("green_digital_clients")
        created_by = (b.get("createdBy") or "").strip()
        shared_col("green_digital_clients").insert_one({"Id": new_id, "ClientName": name, "createdBy": created_by, **update_doc})
        return ok({"success": True, "id": new_id})

@app.get("/api/billing/customers/byname")
async def billing_customer_byname(name: str = Query("")):
    if not name: return err("name required")
    r = shared_col("green_digital_clients").find_one({"ClientName": name}, {"_id": 0})
    if not r:
        r = shared_col("green_digital_clients").find_one({"ClientName": {"$regex": name, "$options": "i"}}, {"_id": 0})
...
async def billing_get_customer(cust_id: int):
    r = shared_col("green_digital_clients").find_one({"Id": cust_id}, {"_id": 0})
    if not r: return err("Not found", 404)
    return JSONResponse(content={"Id": r.get("Id"), "Name": r.get("ClientName",""), **{k: r.get(k,"") for k in ["BillToAddress","ShipToAddress","State","StateCode","Mobile","GSTNo","Email","CustomerType"]}})
...
async def billing_put_customer(cust_id: int, request: Request):
    ...
    existing = shared_col("green_digital_clients").find_one({"Id": cust_id}, {"_id": 0, "ClientName": 1, "Name": 1, "PreviousNames": 1}) or {}
    update_set = {
        "ClientName":    name,
        "Name":          name,
    ...
    result = shared_col("green_digital_clients").update_one({"Id": cust_id}, {"$set": update_set})
    if result.matched_count == 0:
...
async def customer_update_everywhere(cust_id: int, request: Request):
    ...
    """
    if not await ensure_db(): return err("Database not connected", 503)
    cust = shared_col("green_digital_clients").find_one({"Id": cust_id}, {"_id": 0})
    if not cust:
        return err("Customer not found", 404)
...
async def billing_delete_customer(cust_id: int, user: Optional[str] = Query(None), scope: Optional[str] = Query(None)):
    ...
    exists = shared_col("green_digital_clients").count_documents({"Id": cust_id}, limit=1) > 0
    if not exists:
        return err("Customer not found", 404)
    record = shared_col("green_digital_clients").find_one({"Id": cust_id}, {"_id": 0, "createdBy": 1}) or {}
    ...
    shared_col("green_digital_clients").delete_one({"Id": cust_id})
    return ok({"success": True})
...
async def billing_invoice_byno(invno: str = Query(""), fy: str = Query("")):
    ...
    if inv.get("CustomerId"):
        c = shared_col("green_digital_clients").find_one({"Id": inv["CustomerId"]}, {"_id": 0})
        if c:
...
async def billing_get_invoice(inv_id: int):
    ...
    cust = {}
    if r.get("CustomerId"):
        c = shared_col("green_digital_clients").find_one({"Id": r["CustomerId"]}, {"_id": 0})
        if c:
            cust = {"CustomerAddress": c.get("BillToAddress",""), "CustomerState": c.get("State",""),
                    "CustomerStateCode": c.get("StateCode",""), "CustomerMobile": c.get("Mobile",""),
                    "CustomerGST": c.get("GSTNo",""), "CustomerEmail": c.get("Email","")}
...
async def refresh_quotation_customer(qid: str):
    ...
    if not cust:
        ...
        import re as _re
        cust = shared_col("green_digital_clients").find_one(
...
async def login(request: Request):
    ...
    user = _db["green_digital_users"].find_one({"username": username}, {"_id": 0})
    if not user:
    ...
    _db["green_digital_users"].update_one(
        {"username": username},
...
async def get_users():
    ...
    users = list(_db["green_digital_users"].find({}, {"_id": 0, "password": 0, "session_token": 0}))
...
async def create_user(request: Request):
    ...
    if not role:
        role = "guest"
    if col("green_digital_users").find_one({"username": username}):
...
async def update_user(username: str, request: Request):
    ...
    col("green_digital_users").update_one({"username": username}, {"$set": update})
    return ok()
...
async def delete_user(username: str):
    ...
    col("green_digital_users").delete_one({"username": username})
    return ok()
...
async def sync_invoice_from_sales(inv_id: int):
    ...
    hsn = ""; gst_rate = 18.0
    product = col("green_digital_products").find_one(
    ...
    cust_doc = shared_col("green_digital_clients").find_one({"ClientName": sale.get("Customer", "")}, {"_id": 0})
    if cust_doc:
...
async def get_roles():
    if not await ensure_db():
        return JSONResponse(content=[], status_code=503)
    roles = list(col("green_digital_custom_roles").find({}, {"_id": 0}))
...
async def save_role(request: Request):
    ...
    col("green_digital_custom_roles").update_one(
        {"id": role_id},
        {"$set": b},
        upsert=True
    )
...
async def delete_role(role_id: str):
    if not await ensure_db(): return err("Database not connected", 503)
    col("green_digital_custom_roles").delete_one({"id": role_id})
    return ok()
...
async def merge_customers_to_shared():
    ...
    try:
        ...
        rainbow_clients = list(_client[MONGO_DB_RAINBOW]["green_digital_clients"].find({}, {"_id": 0}))
        merged = 0
        skipped = 0
        for c in rainbow_clients:
            ...
            if not name:
                continue
            exists = shared_col("green_digital_clients").find_one({"ClientName": {"$regex": f"^{re.escape(name)}$", "$options": "i"}})
            if exists:
                skipped += 1
                continue
            # Assign new Id
            ...
            c["Id"] = new_id
            c.pop("_id", None)
            shared_col("green_digital_clients").insert_one(c)
            merged += 1
